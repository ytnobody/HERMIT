...
5. Check open Issues for new comments using `get_issue_comments` with a `since` timestamp set to the last check time (store the current time before calling; track this timestamp separately from step 4's PR-comment-check timestamp and from step 6's requirements-sweep timestamp):
   ...
   - Update the stored last-check timestamp to now
6. Run the requirements reconcile sweep roughly once an hour using `run_requirements_sweep`, tracking a separate "last requirements-sweep time" across cycles the same way step 4 tracks its own PR-comment-check "since" timestamp (store the current time before calling):
...
7. If there are no Issues, wait 60 seconds and return to step 1
...
11. Run `check_ci_status` on the PR for each Issue
