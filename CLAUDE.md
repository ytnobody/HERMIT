...
**Tool-scoping guidance (optional hardening, not yet implemented):** the prompt-level prohibition above is the required, load-bearing guard. If the Agent tool in use supports selecting a restricted `subagent_type` (an agent definition with no `Edit`/`Write`/`NotebookEdit` in its tool list — see the general "read-only agent" pattern used elsewhere, e.g. an `Explore`-style agent), prefer dispatching the background Superintendent pass under such a type instead of the unrestricted general-purpose one, so the prohibition is enforced by tool availability and not only by instruction-following. This repo does not currently define such an agent type; adding one (e.g. under `.claude/agents/`) is a reasonable follow-up but is out of scope for this fix.
