...
4. Run the requirements reconcile sweep roughly once an hour using `run_requirements_sweep` — track a "last requirements-sweep time" across cycles yourself (store the current time before calling):
   - Only call `run_requirements_sweep` when at least 3600 seconds have elapsed since the last recorded sweep time; otherwise skip this step for the current cycle
   - Update the stored last-sweep timestamp to now after calling
5. If there are no Issues, stop here (will recheck on next loop)
...
9. Run `evaluate_risk` on the PR for each Issue
