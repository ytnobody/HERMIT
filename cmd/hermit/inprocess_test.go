...
// TestREQ011_ClaudeMdReferencesConfiguredMaxEngineersAsCap verifies the
// second half of REQ-011's acceptance criteria: the CLAUDE.md template
// generated by `hermit init` actually references the harness.toml-configured
// max_engineers value as the parallel-Engineer cap in the Superintendent
// spawn step, not just a hardcoded default.
//
// This complements internal/mcp.TestREQ011_GetConfig_ReturnsMaxEngineers,
// which covers only the get_config half of the acceptance criteria; that
...
func TestREQ011_ClaudeMdReferencesConfiguredMaxEngineersAsCap(t *testing.T) {
	...
	// Use a distinctive, non-default max_engineers value so this test can't
	// pass merely by matching the hardcoded default (4).
	const configuredMaxEngineers = "9"
	...
	go func() {
		...
		fmt.Fprintln(sc, "test-repo")
		fmt.Fprintln(sc, "en")
		fmt.Fprintln(sc, configuredMaxEngineers)
		sc.Flush()
		w.Close()
	}()
	...
	if !strings.Contains(content, "up to "+configuredMaxEngineers+" at a time") {
		t.Errorf("CLAUDE.md parallel-cap step missing configured max_engineers value (%s) in \"up to N at a time\":\n%s", configuredMaxEngineers, content)
	}
	if !strings.Contains(content, "exceeds "+configuredMaxEngineers) {
		t.Errorf("CLAUDE.md spawn step missing configured max_engineers value (%s) in the \"exceeds N\" overflow check:\n%s", configuredMaxEngineers, content)
	}
	...
	if !strings.Contains(string(harnessToml), "max_engineers = "+configuredMaxEngineers) {
		t.Errorf("harness.toml missing configured max_engineers value (%s):\n%s", configuredMaxEngineers, string(harnessToml))
	}
}
