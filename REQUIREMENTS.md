...
- verify: manual
- 実装状況: 実装済み — `cmd/hermit/templates/CLAUDE.md.tmpl` の Superintendent セクション。設計の 7 ステップから大幅に拡張され、`.hermit-quit` / `.hermit-paused` による停止・一時停止、要件ヒアリング Issue の分岐、Issue 粒度チェック、PR/Issue コメント検出、HIGH リスク時の実質レビュー実施などが追加されている
